...
export default function JobTable({ username }: { username: string }) {
  ...
  const [tableSize, setTableSize] = useState('lg')
  ...
  const createTable = (size: string) => {





    return jobList.map((job: Job & { checked: boolean }, index) => {
      ...
      return (
        <tr key={index}>
          <th>
            <label>
              <input checked={job.checked} type="checkbox" className="checkbox" onChange={() => checkJob(job)} />
            </label>
          </th>
          <td>
            {job.title}
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className="overflow-x-auto overflow-y-auto h-screen">
        {/* Toolbar */}
        <div className='join flex flex-wrap justify-between px-2'>
          ...
          <div className='text-sm breadcrumbs'>
            <ul>
              ...
              <li>
                <select className="select select-xs select-ghost w-full max-w-m" onChange={(e) => setTableSize(e.target.value)}>
                  <option disabled defaultValue={"View"}></option>
                  <option value={'sm'}>Condensed</option>
                  <option value={'lg'}>Detailed</option>
                </select>
              </li>
            </ul>
          </div>
